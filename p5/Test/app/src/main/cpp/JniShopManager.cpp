...
long ShopManager::findByLocation(jint x, jint y, jint distance) {
    std::tuple <jint, jint> geek;
    geek = make_tuple(x, y);
    vector<JniShop *> *shops = kdtree->Range_Search(geek, distance);


//    vector<JniShop *> *shops = new vector<JniShop *>();
//    for (auto node: *kdtree) {
//        if ((x - distance) <= T0(node.first) &&
//            T0(node.first) <= (x + distance) &&
//            (y - distance) <= T1(node.first) &&
//            T1(node.first) <= (y + distance)) {
//            shops->push_back(node.second);
//        }
//    }
